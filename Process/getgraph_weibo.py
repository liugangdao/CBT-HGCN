...
def parse_args():
    ...
    parser.add_argument('-obj', type=str, default='Weibo')
...
def hyperEdge(branch,node_count):
    ...
    if branch != []:
        ...
        for i,val in enumerate(branch):
            ...
            for v in val:
                if v<node_count and count_edge1<node_count:
                    edge0_tem.append(v)
...
def convert_example_to_features(example,max_seq_length,tokenizer):
    ...
    return [input_ids,cls_mask,segment_ids]
...
def buildgraph(obj, format_, class_num, portion='', texts=None, labels=None):
    ...
    if class_num == 2:
        labelset_0, labelset_1 = ['non-rumours', 'non-rumor', 'true','0'], ['rumours', 'rumor', 'false','1']
    ...
    for _, line in labels.iterrows():
        eid, label = str(line[0]), str(line[1])
        # label=label.lower()
        event.append(eid)
    ...
    def loadEid(event, id, y, format_):
        ...
        if len(event)>= 1:
            if format_ == 'idx_cnt':
                ...
                [input_ids,input_mask,segment_ids] = convert_example_to_features(x_all_text,300,tokenizer)
                tree,count = filter_edge(input_ids,tree)
                branches = mat2branches(matrix,rootindex)
                hyper_tree = hyperEdge(branches,count)
                assert max(hyper_tree[0])<count
                assert max(hyper_tree[1])<count
                tree, rootindex, y = np.array(hyper_tree), np.array(rootindex), np.array(y)
                input_ids,input_mask,segment_ids = np.array(input_ids),np.array(input_mask),np.array(segment_ids)
                np.savez( os.path.join(cwd, './dataset/'+obj+'textgraph/'+id+'.npz'), input_ids=input_ids,input_mask=input_mask,segment_ids=segment_ids,root=root_text,edgeindex=tree,rootindex=rootindex,y=y)
            return None
