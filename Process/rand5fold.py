...
def load5foldData(obj,labelDic):
    if 'Twitter' in obj:
    ...
    if obj == "Weibo":
        labelPath = os.path.join(cwd,"dataset/Weibo/data.label.txt")
...
def split2save(obj,labelDic,train,test,ind):
    train_fold,test_fold = [],[]
    for val in train:
        train_fold.append(val+'\t'+str(labelDic[val]))
    for val in test:
        test_fold.append(val+'\t'+str(labelDic[val]))
    trainLabel = os.path.join(cwd,'dataset/{}/split_{}/train.label.txt'.format(obj, ind))
    testLabel = os.path.join(cwd,'dataset/{}/split_{}/test.label.txt'.format(obj, ind))
    with open(trainLabel,'w') as f:
        f.write('\n'.join(train_fold))
    with open(testLabel,'w') as f:
        f.write('\n'.join(test_fold))

if __name__=="__main__":
    obj = "Weibo"
    labelDic = {}
    fold = load5foldData(obj,labelDic)
    for i in range(5):
        split2save(obj,labelDic,fold[i*2+1],fold[i*2],i)