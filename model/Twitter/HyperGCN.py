...
class BertForHgcn(PreTrainedBertModel):
    ...
    def __init__(self, config, num_labels=2):
        super(BertForHgcn, self).__init__(config)
        self.num_labels = num_labels
        self.bert = BertModel(config)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        conv =  HypergraphConv(config.hidden_size, config.hidden_size)
        self.layers = nn.ModuleList([copy.deepcopy(conv) for _ in range(config.hypergcn_layers)])
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None,edge=None, labels=None):
        ...
        for layer_module in self.layers:
            input_tensor = layer_module(input_tensor, edge)
