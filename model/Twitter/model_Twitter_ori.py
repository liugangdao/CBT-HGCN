...
from BERT import BertForClassfier
...
def train_fold(train_fold,test_fold):
    ...
    # Prepare model
    model = BertForClassfier.from_pretrained(bert_path,num_labels = num_labels)
    model.to(device)
    # Prepare optimizer
    ...
    for epoch_ids in range(num_train_epochs):
            ...
            for step, batch in enumerate(tqdm(train_loader, desc="Iteration")):
                ...
                loss = model(input_ids, segment_ids, labels=label_ids)
                # if n_gpu > 1:
            ...
            for batch in tqdm(test_loader, desc="Evaluating"):
                ...
                with torch.no_grad():
                    tmp_eval_loss = model(input_ids, segment_ids, labels=label_ids)
...
output_dir = "results/hypergcn/Twitter16"
