from modeling_ori import PreTrainedBertModel,BertModel
import torch
...
class BertForClassfier(PreTrainedBertModel):
    ...
    def __init__(self, config, num_labels=2):
        super(BertForClassfier, self).__init__(config)
        self.num_labels = num_labels
        self.bert = BertModel(config)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        # self.conv = HypergraphConv(config.hidden_size, config.hidden_size)
