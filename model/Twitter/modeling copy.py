...
from Process.file_utils import cached_path
...
class BertEncoder(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask_punk,attention_mask_origin, output_all_encoded_layers=True):
        all_encoder_layers = []
        k = 0
        for layer_module in self.layer:
            if k<4:
                hidden_states = layer_module(hidden_states, attention_mask_punk)
            else:
                hidden_states = layer_module(hidden_states, attention_mask_origin)
...
class BertModel(PreTrainedBertModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, output_all_encoded_layers=True):
        ...
        if attention_mask is None:
            ...
            attention_mask = attention_mask.view(batch_size,seq_length,seq_length)
        if token_type_ids is None:
            token_type_ids = torch.zeros_like(input_ids)
        attention_mask_origin = torch.ones_like(input_ids)
        # We create a 3D attention mask from a 2D tensor mask.
        # Sizes are [batch_size, 1, 1, to_seq_length]
        # So we can broadcast to [batch_size, num_heads, from_seq_length, to_seq_length]
        # this attention mask is more simple than the triangular masking of causal attention
        # used in OpenAI GPT, we just need to prepare the broadcast dimension here.
        extended_attention_mask_punk = attention_mask.unsqueeze(1)
        extended_attention_mask_origin = attention_mask_origin.unsqueeze(1).unsqueeze(2)
        ...
        extended_attention_mask_punk = extended_attention_mask_punk.to(dtype=next(self.parameters()).dtype) # fp16 compatibility
        extended_attention_mask_punk = (1.0 - extended_attention_mask_punk) * -10000.0

        extended_attention_mask_origin = extended_attention_mask_origin.to(dtype=next(self.parameters()).dtype) # fp16 compatibility
        extended_attention_mask_origin = (1.0 - extended_attention_mask_origin) * -10000.0

        embedding_output = self.embeddings(input_ids, token_type_ids)
        encoded_layers = self.encoder(embedding_output,
                                      extended_attention_mask_punk,
                                      extended_attention_mask_origin,
        ...
        cls = 101
        sep = 102
        cls_pos = input_ids==cls
        cls_pos[:,0]=True
        node_batch = cls_pos.sum(axis=[1])
        return encoded_layers, pooled_output,cls_pos,node_batch
